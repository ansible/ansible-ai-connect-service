...
OLLAMA_PIPELINE_CONFIGURATION = TypeVar("OLLAMA_PIPELINE_CONFIGURATION", bound=OllamaConfiguration)
...
def unwrap_role_answer(message: str, create_outline: bool) -> tuple[str, list[dict[str, str]], str]:
...
class OllamaCompletionsPipeline(
    ...
    def invoke(self, params: CompletionsParameters) -> CompletionsResponse:
        ...
        formatted_prompt = self.format_prompt(
            SYSTEM_MESSAGE_TEMPLATE, HUMAN_MESSAGE_TEMPLATE, prompt=full_prompt
        )
...
class OllamaPlaybookGenerationPipeline(
    ...
    def invoke(self, params: PlaybookGenerationParameters) -> PlaybookGenerationResponse:
        ...
        formatted_prompt = self.format_prompt(
            system_template, human_template, text=text, outline=outline
        )
...
class OllamaRoleGenerationPipeline(
    OllamaBase[OLLAMA_PIPELINE_CONFIGURATION, RoleGenerationParameters, RoleGenerationResponse],
    ...
    def invoke(self, params: RoleGenerationParameters) -> RoleGenerationResponse:
        ...
        formatted_prompt = self.format_prompt(
            SYSTEM_MESSAGE_ROLE_REQUEST, HUMAN_MESSAGE_TEMPLATE, text=text, outline=outline
        )
        output = llm.invoke(formatted_prompt)
        ...
        return role, files, outline, []  # type: ignore[return-value]
...
@Register(api_type="ollama")
class OllamaRoleExplanationPipeline(
    OllamaMetaDataMixin[OllamaConfiguration], ModelPipelineRoleExplanation[OllamaConfiguration]
):
    ...
    def invoke(self, params: RoleExplanationParameters) -> RoleExplanationResponse:
        # Role explanation is not yet implemented for Ollama
        from ansible_ai_connect.ai.api.exceptions import FeatureNotAvailable

        raise FeatureNotAvailable

    def self_test(self) -> HealthCheckSummary:
        # Return "nop" as provider since this feature is not implemented
        return HealthCheckSummary(
            {
...
class OllamaPlaybookExplanationPipeline(
    ...
    def invoke(self, params: PlaybookExplanationParameters) -> PlaybookExplanationResponse:
        ...
        formatted_prompt = self.format_prompt(
            SYSTEM_MESSAGE_TEMPLATE, HUMAN_MESSAGE_TEMPLATE, playbook=content
        )
