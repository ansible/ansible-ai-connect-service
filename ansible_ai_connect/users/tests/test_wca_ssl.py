...
class TestWCASSLIsolation(SimpleTestCase):
    ...
    def test_ams_check_ssl_unaffected_by_pipeline_ssl_config(self):
        ...
        with (
            ...
            ams_check = AMSCheck(
                "test_client", "test_secret", "https://sso.redhat.com", "https://api.openshift.com"
            )
    ...
    def test_token_ssl_error_handling(self):
        ...
        with patch("requests.post") as mock_post:
            ...
            with self.assertLogs(
                logger="ansible_ai_connect.users.authz_checker", level="INFO"
            ) as log:
...
class TestWCASSLCertificateScenarios(SimpleTestCase):
    ...
    def test_token_with_custom_server(self):
        ...
        with patch("requests.post") as mock_post:
            ...
            args, kwargs = mock_post.call_args
            expected_url = (
                "https://custom-sso.example.com/auth/realms/redhat-external/"
...
-----END CERTIFICATE-----"""
    ...
    def test_multiple_ssl_context_independence(self):
        """
        Test that multiple WCA authentication instances don't interfere
        with each other's SSL context.
        """
        ...
        ams1 = AMSCheck(
            "client1", "secret1", "https://sso1.redhat.com", "https://api1.openshift.com"
        )
        ams2 = AMSCheck(
            "client2", "secret2", "https://sso2.redhat.com", "https://api2.openshift.com"
        )
        ...
        with (
            ...
            def get_side_effect(url, **kwargs):
                response = Mock()
                hostname = urlparse(url).hostname
                if hostname == "api1.openshift.com":
                    response.json.return_value = {"items": [{"id": "org1"}]}
                elif hostname == "api2.openshift.com":
