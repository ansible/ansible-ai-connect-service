...
class TestSegment(TestCase):
    ...
    @override_settings(ENABLE_ARI_POSTPROCESS=False)
    @override_settings(SEGMENT_WRITE_KEY='DUMMY_KEY_VALUE')
    def test_send_segment_event_commercial_but_not_opt_out_inlineSuggestionFeedback(
        self, track_method
    ):
        g = Mock()
        g.values_list = MagicMock(return_value=[])
        user = Mock(rh_user_has_seat=True, groups=g, organization=Mock(telemetry_opt_out=False))
        event = {
            'rh_user_has_seat': True,
        }
        ...
        with self.assertLogs(logger='root') as log:
            ...
            self.assertEqual(
                log.output[0],
                'INFO:ai.api.utils.segment:sent segment event: inlineSuggestionFeedback',
            )

    @mock.patch("ai.api.utils.segment.analytics.track")
    @override_settings(ENABLE_ARI_POSTPROCESS=False)
    @override_settings(SEGMENT_WRITE_KEY='DUMMY_KEY_VALUE')
    def test_send_segment_event_commercial_and_opt_out_inlineSuggestionFeedback(self, track_method):
        g = Mock()
        g.values_list = MagicMock(return_value=[])
        user = Mock(rh_user_has_seat=True, groups=g, organization=Mock(telemetry_opt_out=True))
        event = {
            'rh_user_has_seat': True,
        }
    ...
    @override_settings(ENABLE_ARI_POSTPROCESS=False)
    @override_settings(SEGMENT_WRITE_KEY='DUMMY_KEY_VALUE')
    def test_send_segment_event_commercial_but_and_opt_out_ansibleContentFeedback(
        self, track_method
    ):
        g = Mock()
        g.values_list = MagicMock(return_value=[])
        ...
        with self.assertLogs(logger='root') as log:
            ...
            self.assertEqual(
                log.output[0],
                'ERROR:ai.api.utils.segment:It is not allowed to track'
                + ' ansibleContentFeedback events for seated users',
            )

    @mock.patch("ai.api.utils.segment.analytics.track")
    @override_settings(ENABLE_ARI_POSTPROCESS=False)
    @override_settings(SEGMENT_WRITE_KEY='DUMMY_KEY_VALUE')
    def test_send_segment_event_commercial_but_not_opt_out_ansibleContentFeedback(
        self, track_method
    ):
        g = Mock()
        g.values_list = MagicMock(return_value=[])
