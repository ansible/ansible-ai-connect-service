...
class TestSegment(TestCase):
    ...
    @override_settings(ENABLE_ARI_POSTPROCESS=False)
    @override_settings(SEGMENT_WRITE_KEY='DUMMY_KEY_VALUE')
    def test_send_segment_event_commercial_but_not_opt_out(self, track_method):
        g = Mock()
        g.values_list = MagicMock(return_value=[])
        org = Mock(telemetry_opt_out=False)
        user = Mock(rh_user_has_seat=True, groups=g, organization=org)
        event = {
            'rh_user_has_seat': True,
        }
    ...
    @override_settings(ENABLE_ARI_POSTPROCESS=False)
    @override_settings(SEGMENT_WRITE_KEY='DUMMY_KEY_VALUE')
    def test_send_segment_event_commercial_but_no_organization(self, track_method):
        g = Mock()
        g.values_list = MagicMock(return_value=[])
        user = Mock(rh_user_has_seat=True, groups=g, organization=None)
        event = {
            'rh_user_has_seat': True,
        }
    ...
    @override_settings(ENABLE_ARI_POSTPROCESS=False)
    @override_settings(SEGMENT_WRITE_KEY='DUMMY_KEY_VALUE')
    def test_send_segment_event_commercial_but_and_opt_out(self, track_method):
        g = Mock()
        g.values_list = MagicMock(return_value=[])
