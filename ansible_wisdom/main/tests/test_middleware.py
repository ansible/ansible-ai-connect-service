    ...
    def test_full_payload(self):
        ...
        with patch.object(
            ...
            with self.assertLogs(logger='root', level='DEBUG') as log:
                ...
                self.assertInLog("'event': 'lightspeedServicePredictionsEvent',", log.output)
                self.assertInLog("'event': 'lightspeedServicePostprocessingEvent',", log.output)
                self.assertInLog("'event': 'lightspeedServiceCompletionEvent',", log.output)
            ...
            with self.assertLogs(logger='root', level='DEBUG') as log:
                ...
                self.assertInLog("DEBUG:segment:queueing:", log.output)
                self.assertNotInLog("'event': 'lightspeedServicePredictionsEvent',", log.output)
                self.assertNotInLog("'event': 'lightspeedServicePostprocessingEvent',", log.output)
                self.assertInLog("'event': 'lightspeedServiceCompletionEvent',", log.output)
                self.assertNotInLog("foo@ansible.com", log.output)
                self.assertNotInLog("username", log.output)
