...
class TransformersClassifierHandler(BaseHandler):
    ...
    def preprocess(self, data):
        ...
        pdata = data[0]
        prefix_prompt = '- name: '
        text = self.tokenizer.bos_token + pdata.get("context") + self.tokenizer.sep_token +\
            prefix_prompt + pdata.get("prompt").strip() + '\n'
    ...
    def inference(self, inputs):
        ...
        print(
            f'source id tensor shape: {source_ids_.shape} \nsize of source id tensor {source_ids_.element_size() * source_ids_.nelement()} and {sys.getsizeof(source_ids_.storage())}')
        ...
        with torch.inference_mode():
            ...
            logger.info(
                f'past the prediction phase, not converting back to decoded {preds}')

            #pad = torch.nn.ZeroPad2d(padding=(self.model_size+self.max_target_length-preds.shape[1],0,0,0))
            #pred_ids = pad(preds)
        ...
        try:
            ...
            logger.error(
                f'failed to split ansible and endoftex token for: {output}')
