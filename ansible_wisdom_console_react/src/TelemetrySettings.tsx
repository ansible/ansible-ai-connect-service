...
export function TelemetrySettings() {
  ...
  const [telemetrySettingsError, setTelemetrySettingsError] =
    useState<HasError>(NO_ERROR);
  const isTelemetryLoading: boolean = useMemo(
    () => _telemetry.status === "NOT_ASKED" || _telemetry.status === "LOADING",
    [_telemetry],
  );
  const isTelemetryFound: boolean = useMemo(
    () => _telemetry.status === "SUCCESS",
    [_telemetry],
  );
  ...
  useEffect(() => {
    ...
    if (_telemetry.status === "SUCCESS") {
      const optOut: boolean = (_telemetry as Success<TelemetryResponse>).data
        .optOut;
  ...
  const optInOutChanged = useMemo(() => {
    if (telemetry.status === "SUCCESS") {
      return (
        (telemetry as Success<TelemetryResponse>).data.optOut !==
        telemetryOptOut
      );
    }
    return false;
              ...
              <Stack hasGutter={true}>
                  ...
                  <TextContent>
                    ...
                    <Text component={"p"}>
                      <a href={t("TelemetryManageUsageLearnMoreURL")}>
                        ...
                        <Icon size="md">
                          <ExternalLinkAltIcon
                            style={{ color: "var(--pf-c-content--a--Color)" }}
                          />
                        </Icon>
                      </a>
                    </Text>
                ...
                <StackItem>
                  {isTelemetryLoading && (
                    <div
                      className={"Loading"}
                      data-testid={"telemetry-settings__telemetry-loading"}
                    >
