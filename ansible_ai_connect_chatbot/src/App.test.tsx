...
async function renderApp (debug = false) {
  ...
  vi.waitFor(async () => {
  await expect.element(page.getByText("Hello, Ansible User")).toBeVisible();
...
test("Basic chatbot interaction", async () => {
  ...
  vi.waitFor(async () => {
  await expect.element(page.getByLabelText("Send button")).toBeVisible();
  });

  await page.getByLabelText("Send button").click();
  //   expect(
  //     await page.getByText(
  //       "In Ansible, the precedence of variables is determined by the order...",
  //     ),
  //   ).toBeVisible();
  //   expect(await page.getByText("Create variables")).toBeVisible();
  //
  ...
  //
  //   const clearContextButton = await page.getByText("Clear context");
  ...
  //   expect(
  //     await page.getByText("While Lightspeed strives for accuracy,", {
  //       exact: false,
  //     }),
  //   ).toBeVisible();
  //   const gotItButton = await page.getByText("Got it");
...
//   mockAxios(200);
//   renderApp();
//   await page.getByText("Send a message...").fill("Hello");
//   await page.getByText("Send button").click();
//   expect(
//     await page.getByText(
//       "In Ansible, the precedence of variables is determined by the order...",
//     ),
...
//   const textArea = await page.getByText("Send a message...");
//   await act(async () => userEvent.type(textArea, "Hello"));
//   const sendButton = await page.getByText("Send button");
...
//   const screen = renderApp(true);
//   await page.getByText("granite3-8b").click();
//   expect(
//     await screen.getByRole("menuitem", { name: "granite3-8b" }),
//   ).toBeTruthy();
//   await act(async () =>
