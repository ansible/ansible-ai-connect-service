...
if [ "${DEPLOYMENT_MODE}" == "upstream" ]; then
    ...
    /var/www/venv/bin/python ansible-wisdom-service/ansible_wisdom/manage.py createtoken --username testuser --password testuser --token-name testuser_token --create-user
fi
